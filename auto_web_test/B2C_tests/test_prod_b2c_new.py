...
class GongbizAutomationTest:
    ...
    async def navigate_to_main(self):
        """메인 페이지로 이동"""
        await self.page.goto("https://zero.gongbiz.kr/main")
    ...
    async def enter_nail_shop(self):
        ...
        try:
            ...
            # "네일포시즌 풋풋한..." 샵 클릭
            shop_selector = "text=ㅎㅔㄹㅔㄴ_b2c..."
            await self.page.click(shop_selector)
            await self.page.wait_for_load_state('networkidle')

            # 샵 정보 확인
            shop_name = "ㅎㅔㄹㅔㄴ_b2c_테스트"
            shop_address = "제주특별자치도 제주시 한경면 노을해안로 1174-1 (고산리, 차귀도팬션), 테스트샵"
            bottom_sheet_text = "ㅎㅔㄹㅔㄴ_b2c_테스트"
    ...
    async def select_service_menu(self):
        ...
        try:
            # 콕예약 - 자동화_콕예약 클릭
            await self.page.click("text=헤렌 콕예약")
            await self.page.wait_for_load_state('networkidle')

            # 시술 정보 확인
            service_name = "헤렌 콕예약"
            service_time = "1시간 소요"
            service_price = "3,000원"
    ...
    async def make_reservation(self):
        ...
        try:
            ...
            # 은솔원장 선택
            await self.page.click("text=공비서 대표원장")

            # 오전 9시 선택
            reserve_time = self.page.get_by_text("오전 10:00").nth(1)
            await reserve_time.click()

            # 바텀시트 변경 확인
            deposit_text = "예약금 3,000원"
    ...
    async def verify_payment_page(self):
        ...
        try:
            # 결제 페이지 정보 확인
            shop_name = "ㅎㅔㄹㅔㄴ_테스트"
            customer_name = "헤렌 테스트#123"
            payment_amount = "3,000원"
            customer_request = "[콕예약] 헤렌 콕예약"
    ...
    async def select_french_powder_service(self):
        ...
        try:
            ...
            # 프렌치/파우더 클릭
            await self.page.click("text=여성컷")
            await self.page.wait_for_load_state('networkidle')
            print("✓ 컷 > 여성컷 서비스 선택")
    ...
    async def select_staff_eunsol(self):
        ...
        try:
            ...
            # 은솔 원장의 [선택] 버튼 클릭
            staff_list = self.page.locator("li:has(p:has-text('이슬 담당자'))").first
