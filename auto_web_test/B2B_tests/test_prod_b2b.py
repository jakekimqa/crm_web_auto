...
class B2BAutomationTest:
    def __init__(self):
        self.base_url = "https://gongbiz.kr/signin"
    ...
    async def test_membership_charge(self):
        ...
        # 제출 버튼은 화면 버전에 따라 텍스트가 달라질 수 있어 fallback으로 처리
        ...
        membership_summary = new_page.locator("dl, li, div").filter(has_text=re.compile(r"정액권")).first
        ...
        print(f"✓ 정액권 잔여 금액 확인: {before_amount:,} -> {after_amount:,} (+200,000)")
        print("=== 정액권 충전 테스트 완료 ===\n")
    ...
    async def test_ticket_charge(self):
        ...
        # 충전 전 티켓 잔여 횟수
        ticket_row = new_page.locator("dl, li, div").filter(has_text=re.compile(r"티켓")).first
        ...
        assert after_count - before_count == 5, (
            f"충전 횟수 불일치: before={before_count}, after={after_count}, diff={after_count-before_count}"
        )
        print(f"✓ 티켓 잔여 횟수 확인: {before_count}회 -> {after_count}회 (+5)")
        print("=== 티켓 충전 테스트 완료 ===\n")
    ...
    async def test_make_reservations(self):
        ...
        for i, reservation in enumerate(reservations, 1):
            ...
            await self.page.locator("#floating-layout button").first.click()
            await self.page.locator("h4:has-text('예약 등록')").first.click()
    ...
    async def test_verify_calendar_reservations(self):
        ...
        for i, reservation in enumerate(expected_reservations, 1):
            ...
            # 예약 시간, 시술, 고객 명 추출
            time_text = await self.page.locator("div.sc-ce9503b1-2 h4").nth(1).inner_text()
            procedure_text = await self.page.locator("p.procedure").first.inner_text()
            name_text = await self.page.locator("h3.sc-23af8e07-3.sc-db7b2fd9-1").first.inner_text()

            # 예약 시간, 시술, 고객명 비교
            assert name_text, expected_reservations['customer']
            assert time_text, expected_reservations['display_time']
            assert procedure_text, expected_reservations['menu_category']
    ...
    async def sales_registrations_1(self):
        ...
        sales_page_title = self.page.locator("h1.sc-23af8e07-1.cVDUdo").first
        title_text = await sales_page_title.inner_text()
        await expect(sales_page_title).to_have_text("매출 등록", timeout=2000)

        #총 금액
        total_amount = self.page.locator("h2.sc-23af8e07-2").first
        total_text = await total_amount.inner_text()
        await expect(total_amount).to_have_text("20,000원", timeout=2000)
        ...
        #오입력 메시지 확인
        async with self.page.expect_event("dialog", timeout=3000) as dlg_info:
            await self.page.locator("button:has-text('매출 저장'):visible").last.click()  # 알림을 띄우는 액션
    ...
    async def sales_registrations_2(self):
        ...
        total_amount = self.page.locator("h2.sc-23af8e07-2").first
        # total_text = await total_amount.inner_text()
        await expect(total_amount).to_have_text("0원", timeout=2000)
        ...
        # 2) 입력칸 로케이터
        # ticket_value = self.page.locator("input[name='티켓']").locator(":visible").first
        ticket_value = await self.page.locator("input.sc-6f338920-0.kjwoTx").first.input_value()
        print(ticket_value)
    ...
    async def sales_registrations_3(self):
        ...
        total_amount = self.page.locator("h2.sc-23af8e07-2").first
        total_text = await total_amount.inner_text()
        await expect(total_amount).to_have_text("10,000원", timeout=2000)
    ...
    async def sales_registrations_4(self):
        ...
        print("====고객명은 :", cust_name.inner_text())

        # 제품 선택
        product_tab = self.page.locator("div.sc-61497733-0:has(h4:has-text('제품'))").locator(":visible").first
        await product_tab.click()
        await self.page.locator("#sale-item-product-group button.select-display:visible").first.click()

        # 그룹 선택
        await self.page.locator(f"button:has-text('미분류')").locator(":visible").first.click()
        await self.page.wait_for_timeout(500)

        # 세부 메뉴 선택
        await self.page.locator(f"button:has-text('01_제품_테스트')").locator(":visible").first.click()
        await self.page.wait_for_timeout(500)
    ...
    async def statistics(self):
        ...
        await self.page.get_by_role("button", name="샵 현황").click()
        await self.page.wait_for_load_state('networkidle')
        ...
        # 상품별 통계
        prod_sta_detail = self.page.locator(".sc-45a967ab-0.idQZsJ").nth(1)
        await prod_sta_detail.click()

        # 1) 기간 선택 버튼 클릭 (캘린더 아이콘 포함된 버튼)
        range_btn = self.page.locator("button.sc-c8a80cb6-2.kxeout:has(svg[icon='reserveCalender'])").locator(":visible").first
        await range_btn.click()

        # 1) 날짜 패널 컨테이너(열려있는 것만) 찾기
        panel = self.page.locator("div.sc-61497733-0.sc-c8a80cb6-0.dvraus.gagRLI:visible").first
        await expect(panel).to_be_visible(timeout=3000)

        # 2) 패널 내부의 '오늘' 버튼 클릭
        today_btn = panel.locator("button:has(h4:has-text('오늘'))").first
        await today_btn.click()

        # 날짜가 바뀌어도 동작: 버튼의 라벨 끝이 "기간 검색"인 것 클릭
        ...
        table = self.page.locator("table.sc-ace70d19-2.ldlIXX")

        #실매출 합계
        total_net_sales = table.locator("thead th:has(div:has-text('실 매출 합계'))").first
        col_idx = await total_net_sales.evaluate("th => Array.from(th.parentElement.children).indexOf(th) + 1")
        cell = table.locator("tbody tr").first.locator(f"td:nth-child({col_idx}) span").first

        await expect(cell).to_have_text("320,000원", timeout=3000)
        total_net_sales_text = await cell.inner_text()
        print("실 매출 합계:", total_net_sales_text)  # 예: "320,000원"


        #시술
        treat_sales = table.locator("thead th:has(div:has-text('시술'))").first
        col_idx = await treat_sales.evaluate("th => Array.from(th.parentElement.children).indexOf(th) + 1")
        cell = table.locator("tbody tr").first.locator(f"td:nth-child({col_idx}) span").first

        await expect(cell).to_have_text("10,000원", timeout=3000)
        treat_sales_text = await cell.inner_text()
        print("시술:", treat_sales_text)  # 예: "200,000원"


        #정액권판매
        memebership_sales = table.locator("thead th:has(div:has-text('정액권 판매'))").first
        col_idx = await memebership_sales.evaluate("th => Array.from(th.parentElement.children).indexOf(th) + 1")
        cell = table.locator("tbody tr").first.locator(f"td:nth-child({col_idx}) span").first

        await expect(cell).to_have_text("200,000원", timeout=3000)
        memebership_sales_text = await cell.inner_text()
        print("정액권 판매 :", memebership_sales_text)  # 예: "200,000원"

        #티켓판매
        ticket_sales = table.locator("thead th:has(div:has-text('티켓 판매'))").first
        col_idx = await ticket_sales.evaluate("th => Array.from(th.parentElement.children).indexOf(th) + 1")
        cell = table.locator("tbody tr").first.locator(f"td:nth-child({col_idx}) span").first

        await expect(cell).to_have_text("100,000원", timeout=3000)
        ticket_sales_text = await cell.inner_text()
        print("정액권 판매 :", ticket_sales_text)  # 예: "200,000원"


        #차감 합계
        total_deduct_sales = table.locator("thead th:has(div:has-text('차감 합계'))").first
        col_idx = await total_deduct_sales.evaluate("th => Array.from(th.parentElement.children).indexOf(th) + 1")
        cell = table.locator("tbody tr").first.locator(f"td:nth-child({col_idx}) span").first

        await expect(cell).to_have_text("60,000원", timeout=3000)
        total_deduct_text = await cell.inner_text()
        print("차감 합계:", total_deduct_text)  # 예: "60,000원"


        #정액권 차감
        membership_deduct = table.locator("thead th:has(div:has-text('정액권 차감'))").first
        col_idx = await membership_deduct.evaluate("th => Array.from(th.parentElement.children).indexOf(th) + 1")
        cell = table.locator("tbody tr").first.locator(f"td:nth-child({col_idx}) span").first

        await expect(cell).to_have_text("20,000원", timeout=3000)
        membership_deduct_text = await cell.inner_text()
        print("정액권 차감:", membership_deduct_text)  # 예: "20,000원"


        #티켓 차감
        ticket_deduct = table.locator("thead th:has(div:has-text('티켓 차감'))").first
        col_idx = await ticket_deduct.evaluate("th => Array.from(th.parentElement.children).indexOf(th) + 1")
        cell = table.locator("tbody tr").first.locator(f"td:nth-child({col_idx}) span").first

        await expect(cell).to_have_text("40,000원", timeout=3000)
        ticket_deduct_text = await cell.inner_text()
        print("티켓 차감:", ticket_deduct_text)  # 예: "40,000원"
    ...
    async def custom_payment_method(self):
        ...
        searched_customer = self.page.locator("li.sc-43cea776-4.eONfsB:has(p:has-text('0001'))").first
        ...
        payment_sta_detail = self.page.locator(".sc-45a967ab-0.idQZsJ").nth(2)
        ...
        statistics_payment_table = self.page.locator("tbody.sc-ace70d19-4.MRMcd").nth(1)
